...
export default function UploadEvidence() {
  ...
  // Pick any document/file (pdf, docx, etc.)
  const handlePickDocument = async () => {
    ...
    if (!result.canceled && result.assets?.[0]) {
      ...
      addEvidence({
        id: `${Date.now()}`,
        name: asset.name,
        sizeLabel: formatBytes(asset.size ?? undefined),
        kind: asset.mimeType?.includes("pdf") ? "pdf" : "file",
      });
    }
  };

  // Placeholder handler for integrations not built yet
  const handleComingSoon = (label: string) => {
    Alert.alert(label, "This upload option is coming soon.");
  };
            ...
            <View style={styles.uploadBox}>
              <View style={styles.uploadRow}>
                {uploadOptions.slice(0, 3).map((option) => (
                  ...
                  >
                    <Ionicons name={option.icon} size={30} color="#3F2A88" />
                  </Pressable>
                ))}
              </View>

              <View style={styles.uploadRowCentered}>
