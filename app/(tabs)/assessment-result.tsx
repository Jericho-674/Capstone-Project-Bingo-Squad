...
function RadarSeries({ center, color, radius, scores }: RadarSeriesProps) {
  ...
  const points = scores.map((score, index) =>
    pointForScore(center, radius, index, totalAxes, score),
  );
...
function RadarChart({ size, scores }: { size: number; scores: number[] }) {
  const center = {
    x: size / 2,
    y: size / 2,
  };
  ...
  const outerPoints = Array.from(
    ...
    (_, axisIndex) =>
      pointForScore(center, radius, axisIndex, totalAxes, MAX_SCORE),
  );
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.chartLabel, styles.topRightLabel]}>
        Collaboration
      </Text>

      <Text style={[styles.chartLabel, styles.bottomRightLabel]}>
        Critical Thinking
      </Text>

      <Text style={[styles.chartLabel, styles.bottomLeftLabel]}>
        Problem Solving
      </Text>

      <Text style={[styles.chartLabel, styles.topLeftLabel]}>Contribution</Text>
    </View>
  );
}
...
export default function AssessmentResultScreen() {
  ...
  const [selfScores, setSelfScores] = useState<number[]>([0, 0, 0, 0, 0]);
  ...
  useEffect(() => {
    const loadResult = async () => {
      if (!reflectionId) {
        console.log("ERROR: reflectionId is missing");
      ...
      try {
        ...
        const selfUrl = `${API_BASE_URL}/api/self-assessments/${reflectionId}`;

        console.log("Self Assessment URL:", selfUrl);

        const selfResponse = await fetch(selfUrl);

        console.log("Self Assessment Status:", selfResponse.status);

        if (selfResponse.ok) {
          const selfData = await selfResponse.json();

          console.log("Self Assessment Data:", selfData);

          setSelfScores([
            Number(selfData.communication) || 0,

            Number(selfData.collaboration) || 0,

            Number(selfData.critical_thinking) || 0,

            Number(selfData.problem_solving) || 0,

            Number(selfData.contribution) || 0,
          ]);
        } else {
          const selfError = await selfResponse.text();

          console.log("Self Assessment API Error:", selfError);
        }
        ...
        const assessmentUrl = `${API_BASE_URL}/api/assessments/${reflectionId}`;
        ...
        if (assessmentResponse.ok) {
          ...
          if (
            assessmentData.assessor_score !== null &&
            assessmentData.assessor_score !== undefined
          ) {
            const score = Number(assessmentData.assessor_score);
          ...
          if (
            ...
            String(assessmentData.feedback).trim() !== ""
          ) {
            setFeedback(assessmentData.feedback);
          } else {
            setFeedback(
          ...
          console.log("Assessment API Error:", assessmentError);

          setAssessorScore(null);

          setFeedback("This reflection has not been assessed yet.");
        }
      } catch (error) {
        ...
        setFeedback("Unable to load assessor feedback.");
      } finally {
        setIsLoading(false);
  ...
  const handleSubmitReflection = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to submit reflection");
      }
      ...
      Alert.alert(
        "Submitted",
        "Your reflection has been submitted successfully. You will now be directed to your Reflection History",
        [
          {
            text: "OK",

            onPress: () => router.replace("/(tabs)/reflection-list"),
          },
        ],
      );
    } catch (error) {
  ...
  const validSelfScores = selfScores.filter((score) => score > 0);

  const selfAverage =
    validSelfScores.length > 0
      ? validSelfScores.reduce((sum, score) => sum + score, 0) /
      ...
      <View style={styles.content}>
        ...
        <View style={styles.heading}>
          <View style={styles.eyebrowRow}>
            ...
            <Text style={styles.eyebrow}>REFLECTION · RESULT</Text>
          </View>
        ...
        <View style={styles.chartCard}>
          <RadarChart size={chartSize} scores={selfScores} />
        ...
        <View style={styles.scoreRow}>
          ...
          <ScoreCard
            ...
            label="Assessor"
            score={assessorScore !== null ? assessorScore.toFixed(1) : "-"}
          />
        </View>
